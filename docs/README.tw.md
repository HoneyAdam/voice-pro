...
- 🔊 頂級語音識別: **Whisper**, **Faster-Whisper**, **Whisper-Timestamped**
- 🎤 零樣本語音複製: **F5-TTS**, **E2-TTS**, **CosyVoice**（包含 **Fun-CosyVoice3** — 支援韓語及其他8種語言）
- 📢 多語言文本轉語音: **Edge-TTS**, **kokoro**（可使用您自己的金鑰啟用 **Azure TTS** — 請參閱下方「Azure 服務」章節）
- 🎥 YouTube處理與音頻提取: **yt-dlp**
- 🌍 超過100種語言的即時翻譯: **Deep-Translator**（可使用您自己的金鑰啟用 **Azure Translator**）
...
- **故障排除**: 在大多數情況下，刪除`installer_files`資料夾後重新運行`start.bat`即可解決問題（全新重新安裝只需幾分鐘；已下載到`model/`的AI模型會保留）。錯誤會在WebUI中以紅色提示框顯示，並保持到您關閉為止。
...
- ⚡ **安裝程式從Miniconda/pip遷移至[uv](https://docs.astral.sh/uv/)** — 安裝速度大幅提升，並可從已提交的`uv.lock`完全重現安裝。所有內容都保留在`installer_files/`內（uv、Python、套件）。
- 🐍 升級執行環境：**Python 3.12、Torch 2.8.0+cu128（支援RTX 50系列）、Gradio 6.20**。
- 🎙️ 最新ASR堆疊：**faster-whisper 1.2.1**（large-v3-turbo、distil-large-v3.5）、openai-whisper 20250625、whisper-timestamped 1.15.9。whisperX已被移除（其相依套件的版本鎖定阻礙了Gradio 6升級；既有設定會自動改用faster-whisper）。
- 🗣️ 最新TTS堆疊：**F5-TTS 1.1.21**、kokoro 0.9.4、edge-tts 7.x，並重新整合了上游main版本的**CosyVoice**。
- 🇰🇷 新增可選TTS模型：**Fun-CosyVoice3-0.5B** — 支援包含韓語在內的9種語言，可在CosyVoice標籤頁中選擇（首次使用時會從官方HF儲存庫下載）。
- 🧹 **不再需要**CUDA Toolkit和Visual Studio Build Tools — 所有相依套件均提供預先建置的wheel，且PyTorch內建CUDA執行階段。
- 🛡️ 對**受限/公司電腦**友善：無需管理員權限 — 若未安裝**ffmpeg**，`start.bat`會自動下載可攜式版本；Whisper模型下載在傳輸中斷/損毀後可自我修復；當網路對免費的Google端點進行速率限制時，翻譯會自動以退避方式重試（失敗的行會被回報，並保留原文）。
- 🚨 **錯誤現在會顯示在WebUI中**：每個失敗都會顯示紅色錯誤提示框，並保持在畫面上直到您關閉為止（以前是容易錯過的10秒警告），並針對常見原因（缺少ffmpeg、未登錄媒體等）提供可操作的訊息。
- 🖥️ UI：遷移至**Gradio 6**（所有標籤頁採用全寬版面配置，字幕軌道直接顯示在影片播放器中）。
- 🧽 `uninstall.bat`不再需要管理員權限，也不再強制重新開機；`uninstall.bat silent`可無人值守執行。
...
### 2. 語音技術
- **語音轉文本:** **Whisper**, **Faster-Whisper**, **Whisper-Timestamped**
- **文本轉語音:** 
...
## 💻 系統需求
- **作業系統：** Windows 10/11（64位元）、Linux、Mac（Apple Silicon）
- **顯示卡：** 配備較新驅動程式的NVIDIA顯示卡（建議570以上；支援RTX 50系列）。無需安裝CUDA Toolkit。
- **顯示記憶體：** 4GB以上（建議8GB以上）
- **記憶體：** 4GB以上
- **儲存空間：** 20GB以上可用空間
- **網路：** 必需
...
### 2. 安裝和執行
1. 🚀 **configure.bat**（可選）
   - 在系統範圍內安裝git和ffmpeg（不再需要CUDA Toolkit / Visual Studio）
   - 需要管理員權限；只需執行一次
   - 沒有管理員權限？可以跳過 — **start.bat**會自動下載可攜式ffmpeg
2. 🚀 **start.bat**
   - 啟動Voice-Pro網頁介面
   - 首次執行時會下載uv + Python 3.12，並從lockfile安裝所有相依套件（只需幾分鐘，而非數小時），然後下載AI模型（約10GB — 這是最耗時的部分）
   - 自動偵測GPU/CPU；可透過`GPU_CHOICE`環境變數（`G`=NVIDIA，`C`=CPU）或刪除`installer_files\gpu_choice.txt`來覆寫
   - 如果出現問題，刪除**installer_files**後重試

### 3. 更新
- 🚀 **update.bat**：將Python環境精確地重新同步至已提交的lockfile（快速）

### 4. 解除安裝
- 執行**uninstall.bat**或刪除資料夾（可攜式安裝）
- 無需管理員權限；加上`silent`即可無人值守移除（`uninstall.bat silent`）
- 只會移除`installer_files`資料夾 — 您的`model/`和`workspace/`資料夾會保留

## 🔑 Azure服務（可選，.env）

Voice-Pro預設使用**免費**服務：翻譯使用Deep-Translator（Google的免費網頁端點），語音合成使用Edge-TTS。如果您擁有自己的**Microsoft Azure**訂閱，可以將兩者都切換至Azure API：

1. 將專案根目錄中的`.env.example`複製為`.env`：
   ```bash
...
2. 填入您的Azure憑證：
   ...
   ```
3. 重新啟動Voice-Pro。啟動時會自動偵測有效的金鑰 — 翻譯會切換至**Azure Translator**，語音生成的第一個標籤頁會變為**Azure-TTS**。

**什麼情況下值得設定？**
- 🏢 **公司/受限網路**：資安設備經常對免費的`translate.google.com`端點進行速率限制或封鎖，導致長字幕翻譯變慢或失敗。Voice-Pro會以退避方式重試，並保留失敗行的原文（您會看到帶有失敗數量的警告），但使用Azure Translator可以完全避免這個問題。
- 🗣️ 更高品質、更一致的TTS聲音，以及更高的速率限制。
- **切勿**將`.env`提交至版本控制 — 其中包含您的私人金鑰。
